...
<div class="rounded-2xl border bg-white p-6 max-w-xl">
  <form method="POST" action="{{ route('products.store') }}" class="space-y-4">
    ...
    <div>
      ...
      <input name="nama" class="w-full rounded-xl border px-3 py-2" required>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <input type="number" name="harga" min="0" value="0" class="w-full rounded-xl border px-3 py-2" required>
        @error('harga') <div class="text-sm text-red-600">{{ $message }}</div> @enderror
      </div>
      <div>
        <label class="text-sm font-semibold">Stok</label>
        <input type="number" name="stok" min="0" value="0" class="w-full rounded-xl border px-3 py-2" required>
        @error('stok') <div class="text-sm text-red-600">{{ $message }}</div> @enderror
      </div>
    </div>

    <div class="flex gap-2">
      <button class="rounded-xl bg-indigo-600 text-white px-4 py-2 font-semibold hover:opacity-90">Simpan</button>
